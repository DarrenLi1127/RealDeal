...
export async function postMockApi(page: Page) {
  ...
  await page.route('**/api/posts/user/**', (route) => {
    route.fulfill({
      ...
      contentType: 'application/json',
      body: JSON.stringify({
        content: Array(5).fill(0).map((_, i) => ({
          id: `my_post_${i}`,
          userId: 'user_test123',
          username: 'testuser',
          title: `My Post ${i + 1}`,
          content: `This is my own post ${i + 1}. I created this post for testing.`,
          images: [{ id: `img_${i}`, position: 0, url: 'https://example.com/test-image.jpg' }],
          createdAt: new Date(Date.now() - i * 3600000).toISOString(),
          likesCount: Math.floor(Math.random() * 10),
          starsCount: Math.floor(Math.random() * 5),
  ...
  await page.route('**/api/posts/liked/**', (route) => {
    route.fulfill({
      ...
      contentType: 'application/json',
      body: JSON.stringify({
        content: Array(4).fill(0).map((_, i) => ({
          id: `liked_post_${i}`,
          userId: `other_user_${i}`,
          username: `otheruser${i}`,
          title: `Liked Post ${i + 1}`,
          content: `This is a post I've liked ${i + 1}. Content from another user.`,
          images: [{ id: `img_${i}`, position: 0, url: 'https://example.com/test-image.jpg' }],
          createdAt: new Date(Date.now() - i * 3600000).toISOString(),
          likesCount: 5 + i,
          starsCount: i,
    ...
    route.fulfill({
      ...
      body: JSON.stringify({
        content: Array(3).fill(0).map((_, i) => ({
          ...
          content: `This is a post I've starred ${i + 1}. Special content that I found valuable.`,
          images: [{ id: `img_${i}`, position: 0, url: 'https://example.com/test-image.jpg' }],
          createdAt: new Date(Date.now() - i * 3600000).toISOString(),
          likesCount: i,
          starsCount: 7 + i,
    ...
    route.fulfill({
      ...
      body: JSON.stringify({
        content: Array(4).fill(0).map((_, i) => ({
          ...
          content: `This post matches your search for "${query}". Result ${i + 1}.`,
          images: [{ id: `img_${i}`, position: 0, url: 'https://example.com/test-image.jpg' }],
          createdAt: new Date(Date.now() - i * 3600000).toISOString(),
          likesCount: Math.floor(Math.random() * 10),
          starsCount: Math.floor(Math.random() * 5),
